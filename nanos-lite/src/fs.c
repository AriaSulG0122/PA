...
off_t fs_lseek(int fd, off_t offset, int whence){
	off_t update=file_table[fd].open_offset;
	switch(whence){
		case SEEK_CUR:
			update += offset;
			break;
		case SEEK_SET:
			...
			break;
		case SEEK_END:
			update = file_table[fd].size + offset;
			break;
		default:
			assert(0);
			break;
	...
	update = update > file_table[fd].size? 
				file_table[fd].size: update;

	file_table[fd].open_offset = update;
	return update;
}
