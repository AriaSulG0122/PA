...
bool new_wp(char *args){
  ...
  bool success;
  wp_new->cur_value=expr(wp_new->tar,&success);
  if(!success){
...
void print_wp(){
  ...
  WP *tempNode=head;
  while(tempNode!=NULL){
    printf("No:%d   Target:%s   TitTimes:%d   CurrentValue:%d",tempNode->NO,tempNode->tar,tempNode->hitNum,tempNode->cur_value);
